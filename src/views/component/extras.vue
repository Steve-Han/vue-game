      ...
      <div class="drawer-update" v-if="showExtrasInfo">
        ...
        <div class="scroll">
          <div class="info" v-for="(item,index) in update" :key="index">
            ...
            <h2 class="vision" v-for="(v) in item.vision" :key="v.id" v-if="item.vision">{{ v.vision }}:<a
                :href="v.href" target="_blank"> {{ v.href }}</a></h2>
        ...
        <div class="footer">
          <div class="footer-github">
            <a class="github" target="_blank" @click="navToGithub" title="源码"
               src="https://github.com/Couy69/vue-idle-game">
            </a>
          ...
          <div class="footer-suggest">
            <div>
              <textarea placeholder="发现了bug?抑或是有什么建议？欢迎提出来。" @keydown="eastereEgg1($event)"
                        v-model="suggest"/>
            </div>
            <div>
...
let checkedUpdateInfo = ref(false)
let showExtrasInfo = ref(false)
let name = ref('')
let suggest = ref('')
let disabled = ref(false)
let keyCode = reactive([38, 38, 40, 40, 37, 39, 37, 39, 66, 65, 66, 65])
let reKeyCode = reactive([])
let update = reactive([
  {
    title: '历史版本',
    vision: [{
...
function drawerOpen() {
  showExtrasInfo.value = true
  checkedUpdateInfo.value = true
  localStorage.setItem('version', '1.2.2')
}

function closePanel() {
  localStorage.setItem('version', '1.2.2')
  showExtrasInfo.value = false
}
