<template>
  <div class="neck">
    ...
    <div class="neckPanel" :style="{'box-shadow':' 0 0 5px 5px '+neck.quality.color + 'b8'}"
         v-if="JSON.stringify(neck)!='{}'">
      <div class="title">
        <div class='icon' :class="{'red-flash':neck.enchantlvl>=13,unique:neck.quality.name=='独特'}"
             :style="{'box-shadow':'inset 0 0 7px 2px '+neck.quality.color}">
          <img :src="neck.type.iconSrc" alt="">
        </div>
        <div class='name' :style="{color:neck.quality.color}">{{ neck.type.name }}
          {{ neck.enchantlvl ? '(+' + neck.enchantlvl + ')' : '' }}
        </div>
      </div>
      <div class='type'>
      ...
      <div class="entry">
        <div v-for="v in neck.type.entry" :key="v.id">
          ...
          <div>{{ v.name }} : {{ v.showVal }} <span style="color:#68d5ed"
                                                    v-if="neck.enchantlvl">(+{{
              Math.round(v.value * (1.05 ** (neck.enchantlvl) ** 1.1) - v.value)
            }})</span>
          </div>
        </div>
      </div>
